...
if($category == "0"){
    ...
    echo ("Invalid input for Delivery cost for out of Colombo");
}else if(empty($desc)){
    echo ("Please Enter the Description");
}else{

    $mhb_rs = Database::select("SELECT * FROM `brand_has_model` WHERE `brand_id`='".$brand."' AND `model_id`='".$model."' ");
    ...
    Database::iud("INSERT INTO `product` (`category_id`,`brand_has_model_id`,`price`,`colour_id`,`qty`,`description`,`title`,`condition_id`,`status_id`,`user_email`,`datetime_added`,`delivery_fee_colombo`,`delivery_fee_other`)
     VALUES ('".$category."','".$brand_has_model_id."','".$cost."','".$clr."','".$qty."','".$desc."','".$title."','".$condition."','".$status."','".$email."','".$date."','".$dwc."','".$doc."') ");
